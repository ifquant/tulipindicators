...
- `imi` 虽然名字像常见动量振荡器，但本质是 `open/close` 双输入窗口求和，最适合直接放进 Tulip 现有 indicator 体系
- `sarext` 不能偷懒复用 `psar` 名字映射，因为它额外定义了初始方向、反转偏移、长短分离 AF，以及“空头输出为负值”的 TA-Lib 语义
- 这一批还要求本地 TA-Lib oracle 支持 `price` 类型输入，否则 `sarext` 无法做真正的本地 parity
