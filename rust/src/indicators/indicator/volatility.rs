...
impl Indicator for Volatility {
    ...
    fn run(&self, inputs: &[&[Real]], options: &[Real]) -> Result<Vec<Vec<Real>>, IndicatorError> {
        ...
        let produced = run_volatility_batch(input, period, &mut output);
...
fn run_volatility_batch(input: &[Real], period: usize, output: &mut [Real]) -> usize {
    if input.len() <= period {
        return 0;
    }
    ...
    for index in 1..=period {
        let change = input[index] / input[index - 1] - 1.0;
        sum += change;
        sum2 += change * change;
    }

    let mean = sum * scale;
    output[0] = (sum2 * scale - mean * mean).sqrt() * ANNUAL_SCALE;
    ...
    for (dst, index) in output[1..].iter_mut().zip((period + 1)..input.len()) {
        ...
        let previous_change = input[index - period] / input[index - period - 1] - 1.0;
        sum -= previous_change;
        sum2 -= previous_change * previous_change;

        let next_mean = sum * scale;
        *dst = (sum2 * scale - next_mean * next_mean).sqrt() * ANNUAL_SCALE;
    }
